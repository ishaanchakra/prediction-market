...
function categorizeMarket(question) {
  ...
  const sportsKeywords = ['hockey', 'basketball', 'football', 'lacrosse', 'baseball',
    'soccer', 'rowing', 'beat', 'win', 'game', 'match', 'playoff', 'ecac', 'ncaa',
    'championship', 'frozen four', 'olympic', 'medal', 'varsity', 'tournament'];
  if (sportsKeywords.some((k) => lower.includes(k))) return 'sports';
  return 'wildcard'; // seed script only creates sports markets
}
...
async function seedSportsMarkets() {
  ...
  for (let i = 0; i < upcomingGames.length; i += 1) {
    ...
    try {
      ...
      if (dryRun) {
        created += 1;
        console.log(`         → DRY RUN (would create, category: ${category})\n`);
        continue;
      }
      ...
      created += 1;
      console.log(`         → CREATED (market id: ${docRef.id}, category: ${category})\n`);
    } catch (error) {
      errors += 1;
