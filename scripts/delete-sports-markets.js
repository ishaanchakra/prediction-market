...
// For CANCELLED markets:     already refunded — deletes market + bets.
// For RESOLVED markets:      payouts already settled — deletes market doc only.
//
...
// ─── Refund logic (mirrors utils/refunds.js) ─────────────────────────────────
...
async function deleteSportsMarkets() {
  console.log(`\n🗑️  Delete auto-generated sports markets${isDryRun ? ' (DRY RUN)' : ''}\n`);
  ...
  // ── Step 2: Refund users for OPEN / LOCKED markets ───────────────────────
  ...
  if (needRefund.length > 0) {
    ...
    for (const market of needRefund) {
      ...
      console.log(`  ✓ Refunded ${userIds.length} users for: ${market.question}`);
    }

    console.log(`\nTotal refunded: $${round2(totalRefunded).toFixed(2)}\n`);
  }

  // ── Step 3: Delete all auto-generated markets and their bets ─────────────
