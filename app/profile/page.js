...
export default function ProfilePage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto bg-[var(--bg)] min-h-screen">
      <div className="mb-8">
        <h1 className="mb-2 font-display text-[2rem] leading-[1.2] text-[var(--text)]">Your Profile</h1>
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-[8px] border border-[var(--border)] bg-[var(--surface)] p-6">
          ...
          <p className="mt-2 font-mono text-[0.6rem] text-[var(--text-muted)]">
            Net: {lifetimeNet >= 0 ? '+' : '-'}${Math.abs(lifetimeNet).toFixed(2)} · Week: {weeklyNet >= 0 ? '+' : '-'}${Math.abs(weeklyNet).toFixed(2)}
          </p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {bets.map((bet) => (
            <Link
              ...
              className="block rounded-[6px] border border-[var(--border)] bg-[var(--surface)] p-4 transition-colors hover:border-[var(--border2)] hover:bg-[var(--surface2)]"
            >
              ...
              <p className="mb-1 font-mono text-[var(--amber-bright)]">Amount: <span className="font-semibold">${round2(Math.abs(bet.amount || 0))}</span></p>
